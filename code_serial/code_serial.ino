...
void loop()
{
  ...
  if(Serial.available())
  {
    ...
    if(SC == 'P')
    {
      Serial.print("Enter no. of push ups");
      SC = Serial.Read();
      pushUps((int)SC-48);
    }
    else if(SC == 'T')
    {
      Serial.print("Enter no. of twists");
      SC = Serial.Read();
      twisting((int)SC-48);
    }
    else if(SC == 'Q') bent();
    ...
    {
      Serial.print("Enter no. of shots");
      SC = Serial.Read();
      gun_start((int)SC-48);
    }
    else if(SC == 'C')  meanToDown();
    else if(SC == 'X')  meanToUp();
    else if(SC == 'W')
    {
      Serial.print("Enter no. of steps");
      SC = Serial.Read();
      forward((int)SC-48);
    }
    else if(SC == 'A')rightTurn(5);
    else if(SC == 'D')leftTurn(5);
    else if(SC == 'S')leftTurn(9);
  }
  }
  /*startingCalibrate();
  calibrate();
